...
window.addEventListener('DOMContentLoaded', () => {
	...
	document
		.querySelectorAll('#formatGroup                >*')
		.forEach((e: HTMLElement) => e.addEventListener('click', setFormat)); // Format buttons

	// Hover
	document
		.querySelectorAll('.module-selector')
		.forEach((e: HTMLElement) => e.addEventListener('mouseover', moduleHover));
	document.querySelectorAll('.module-selector').forEach((e: HTMLElement) => e.addEventListener('focus', moduleHover));
	// Mouse leave
	document
		.querySelectorAll('.module-selector')
		.forEach((e: HTMLElement) => e.addEventListener('mouseleave', moduleMouseLeave));
	document
		.querySelectorAll('.module-selector')
		.forEach((e: HTMLElement) => e.addEventListener('blur', moduleMouseLeave));

	// Modules
	document
		.querySelectorAll('#normalModules              >*')
		.forEach((e: HTMLElement) => e.addEventListener('click', setModule)); // Normal modules
	document
		.querySelectorAll('#normalModulesConflicting   >*')
		.forEach((e: HTMLElement) => e.addEventListener('click', setModuleConflicting)); // Conflicting normal modules

	// HUD
	document
		.querySelectorAll('#hudModules                 >*')
		.forEach((e: HTMLElement) => e.addEventListener('click', setIconModule)); // Normal modules
	document
		.querySelectorAll('#hudModulesConflicting      >*')
		.forEach((e: HTMLElement) => e.addEventListener('click', setIconModuleConflicting)); // Conflicting normal modules

	// Options and Panoramas
	document
		.querySelectorAll('#optionsbgModules           >*')
		.forEach((e: HTMLElement) => e.addEventListener('click', setBackground)); // Backgrounds
	document
		.querySelectorAll('#panoramasModules           >*')
		.forEach((e: HTMLElement) => e.addEventListener('click', setPano)); // Panoramas
});
...
function moduleHover(this: HTMLElement) {
	...
	if (this.getAttribute('data-conflicting') != null) {
		...
		if (conflicts.some((r) => modules.includes(r))) {
			// Make popover text
			let styledConflicts = 'Cannot use because you have selected the following: ';
...
function setModule(this: HTMLElement) {
	if (
		!(
			this.getAttribute('data-conflicting') != null &&
			this.getAttribute('data-conflicting').split(' ').some((r) => modules.includes(r))
		)
	) {
...
function setModuleConflicting(this: HTMLElement) {
	if (
		...
		Array.prototype.filter
			.call(this.parentNode.children, (c: HTMLElement) => {
				return c !== this;
			})
...
function setBackground(this: HTMLElement) {
	...
	Array.prototype.filter
		...
		.forEach((e: HTMLElement) => e.classList.remove('enabled'));
...
function disableDownload() {
	shouldDisableDl =
		modules.length == 0 && iconModules.length == 0 && optionsBackground == undefined && panoOption == undefined;
